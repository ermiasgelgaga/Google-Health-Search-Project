...
// 1.2 YEa SELECTION DROPDOWN TWO
// Execution function for second dropdown acting as a cascading dropdown
function optionCompareChangedTwo(select) {
    ...
    d3.json("/allsearchrecord").then((importData) => {
        ...
        // default selection
        var defaultYearID = filterSelect[0].food_code
        var defaultYearSeq_num = filterSelect[0].seq_num
...
function health_case_one(id, select) {
    //Rename variables
    var id_num = id
    var sel = select
    d3.json("/allsearchrecord").then((importData) => {

        // Searching health cases through json
        var cases = importData.data;

        // Empty list to store location_id data
        getLocationIds = []

        // Use filter() to pass selection as an argument for the location_id
        getLocationIds = cases.filter(data => data.location_id == id_num);

        // empty list to get years of food code
        var filterYears = []
        ...
        city = filterYears[0]
        // cancer list to use in our calculation
        cancer = city.portion_weight_g
